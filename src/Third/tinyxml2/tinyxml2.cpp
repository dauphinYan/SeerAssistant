...
char* XMLDocument1::Identify( char* p, XMLNode** node, bool first )
{
    TIXMLASSERT( node );
    TIXMLASSERT( p );
    char* const start = p;
...
bool XMLDocument1::Accept( XMLVisitor* visitor ) const
{
    TIXMLASSERT( visitor );
...
XMLNode::XMLNode( XMLDocument1* doc ) :
    _document( doc ),
    _parent( 0 ),
    _value(),
    _parseLineNum( 0 ),
...
{
    // Edge case: XMLDocument1s don't have a Value. Return null.
    if ( this->ToDocument() )
        return 0;
...
XMLNode* XMLNode::DeepClone(XMLDocument1* target) const
{
...
char* XMLNode::ParseDeep( char* p, StrPair* parentEndTag, int* curLineNumPtr )
{
	...
	XMLDocument1::DepthTracker tracker(_document);
	if (_document->Error())
		return 0;
...
XMLNode* XMLText::ShallowClone( XMLDocument1* doc ) const
{
    if ( !doc ) {
        doc = _document;
    }
...
XMLComment::XMLComment( XMLDocument1* doc ) : XMLNode( doc )
{
}
...
XMLNode* XMLComment::ShallowClone( XMLDocument1* doc ) const
{
    if ( !doc ) {
        doc = _document;
    }
...
XMLDeclaration::XMLDeclaration( XMLDocument1* doc ) : XMLNode( doc )
{
}
...
XMLNode* XMLDeclaration::ShallowClone( XMLDocument1* doc ) const
{
    if ( !doc ) {
        doc = _document;
    }
...
XMLUnknown::XMLUnknown( XMLDocument1* doc ) : XMLNode( doc )
{
}
...
XMLNode* XMLUnknown::ShallowClone( XMLDocument1* doc ) const
{
    if ( !doc ) {
        doc = _document;
    }
...
// --------- XMLElement ---------- //
XMLElement::XMLElement( XMLDocument1* doc ) : XMLNode( doc ),
    _closingType( OPEN ),
    _rootAttribute( 0 )
{
}
...
XMLNode* XMLElement::ShallowClone( XMLDocument1* doc ) const
{
    if ( !doc ) {
        doc = _document;
    }
...
const char* XMLDocument1::_errorNames[XML_ERROR_COUNT] = {
    "XML_SUCCESS",
    "XML_NO_ATTRIBUTE",
...
XMLDocument1::XMLDocument1( bool processEntities, Whitespace whitespaceMode ) :
    XMLNode( 0 ),
    _writeBOM( false ),
...
XMLDocument1::~XMLDocument1()
{
    Clear();
}


void XMLDocument1::MarkInUse(const XMLNode* const node)
{
	TIXMLASSERT(node);
	TIXMLASSERT(node->_parent == 0);
...
void XMLDocument1::DeepCopy(XMLDocument1* target) const
{
	TIXMLASSERT(target);
    if (target == this) {
...
XMLElement* XMLDocument1::NewElement( const char* name )
{
...
XMLComment* XMLDocument1::NewComment( const char* str )
{
...
XMLText* XMLDocument1::NewText( const char* str )
{
    XMLText* text = CreateUnlinkedNode<XMLText>( _textPool );
    text->SetValue( str );
    return text;
...
XMLDeclaration* XMLDocument1::NewDeclaration( const char* str )
{
...
XMLUnknown* XMLDocument1::NewUnknown( const char* str )
{
...
void XMLDocument1::DeleteNode( XMLNode* node )	{
    TIXMLASSERT( node );
...
XMLError XMLDocument1::LoadFile( const char* filename )
{
    if ( !filename ) {
        TIXMLASSERT( false );
...
XMLError XMLDocument1::LoadFile( FILE* fp )
{
    Clear();
...
XMLError XMLDocument1::SaveFile( const char* filename, bool compact )
{
    if ( !filename ) {
        TIXMLASSERT( false );
        SetError( XML_ERROR_FILE_COULD_NOT_BE_OPENED, 0, "filename=<null>" );
...
XMLError XMLDocument1::SaveFile( FILE* fp, bool compact )
{
...
XMLError XMLDocument1::Parse( const char* xml, size_t nBytes )
{
    Clear();
...
void XMLDocument1::Print( XMLPrinter* streamer ) const
{
    if ( streamer ) {
        Accept( streamer );
    }
...
void XMLDocument1::SetError( XMLError error, int lineNum, const char* format, ... )
{
...
/*static*/ const char* XMLDocument1::ErrorIDToName(XMLError errorID)
{
...
const char* XMLDocument1::ErrorStr() const
{
	return _errorStr.Empty() ? "" : _errorStr.GetStr();
}


void XMLDocument1::PrintError() const
{
    printf("%s\n", ErrorStr());
}

const char* XMLDocument1::ErrorName() const
{
    return ErrorIDToName(_errorID);
}

void XMLDocument1::Parse()
{
...
bool XMLPrinter::VisitEnter( const XMLDocument1& doc )
{
