...
/**
	...
	Generally Accept() is called on the XMLDocument1, although all nodes support visiting.
...
public:
    ...
    /// Visit a document.
    virtual bool VisitEnter( const XMLDocument1& /*doc*/ )			{
        return true;
    }
    /// Visit a document.
    virtual bool VisitExit( const XMLDocument1& /*doc*/ )			{
        return true;
    }
...
// WARNING: must match XMLDocument1::_errorNames[]
enum XMLError {
    XML_SUCCESS = 0,
    XML_NO_ATTRIBUTE,
...
/** XMLNode is a base class for every object that is in the
	...
	be navigated. A node is always in a XMLDocument1.
	The type of a XMLNode can be queried, and it can
	be cast to its more defined type.

	A XMLDocument1 allocates memory for all its Nodes.
	When the XMLDocument1 gets deleted, all its Nodes
...
{
    friend class XMLDocument1;
    friend class XMLElement;
public:

    /// Get the XMLDocument1 that owns this XMLNode.
    const XMLDocument1* GetDocument() const	{
        TIXMLASSERT( _document );
        return _document;
    }
    /// Get the XMLDocument1 that owns this XMLNode.
    XMLDocument1* GetDocument()				{
        TIXMLASSERT( _document );
        return _document;
    }
    ...
    virtual XMLDocument1*	ToDocument()	{
        return 0;
    }
    ...
    virtual const XMLDocument1*		ToDocument() const		{
        return 0;
    }
    ...
    */
    ...
    /**
    	...
    	Note: if called on a XMLDocument1, this will return null.
    */
    virtual XMLNode* ShallowClone( XMLDocument1* document ) const = 0;
	...
	/**
        ...
        specified XMLDocument1.

		NOTE: This is probably not the correct tool to
		copy a document, since XMLDocument1s can have multiple
		top level XMLNodes. You probably want to use
        XMLDocument1::DeepCopy()
	*/
	XMLNode* DeepClone( XMLDocument1* target ) const;

    /**
    	Test if 2 nodes are the same, but don't test children.
    	The 2 nodes do not need to be in the same Document.

    	Note: if called on a XMLDocument1, this will return false.
    */
...
protected:
    explicit XMLNode( XMLDocument1* );
    virtual ~XMLNode();

    virtual char* ParseDeep( char* p, StrPair* parentEndTag, int* curLineNumPtr);

    XMLDocument1*	_document;
    XMLNode*		_parent;
...
public:
    ...
    virtual XMLNode* ShallowClone( XMLDocument1* document ) const override;
    virtual bool ShallowEqual( const XMLNode* compare ) const override;

protected:
    explicit XMLText( XMLDocument1* doc )	: XMLNode( doc ), _isCData( false )	{}
    virtual ~XMLText()												{}
...
protected:
    explicit XMLComment( XMLDocument1* doc );
    virtual ~XMLComment();
...
protected:
    explicit XMLDeclaration( XMLDocument1* doc );
...
protected:
    explicit XMLUnknown( XMLDocument1* doc );
    virtual ~XMLUnknown();
...
*/
class TINYXML2_LIB XMLDocument1 : public XMLNode
{
    friend class XMLElement;
...
public:
    /// constructor
    XMLDocument1( bool processEntities = true, Whitespace whitespaceMode = PRESERVE_WHITESPACE );
    ~XMLDocument1();

    virtual XMLDocument1* ToDocument() override		{
        TIXMLASSERT( this == _document );
        return this;
    }
    virtual const XMLDocument1* ToDocument() const override {
        TIXMLASSERT( this == _document );
        return this;
    }
	...
	*/
	void DeepCopy(XMLDocument1* target) const;
...
private:
    XMLDocument1( const XMLDocument1& );	// not supported
    void operator=( const XMLDocument1& );	// not supported
	...
	class DepthTracker {
	public:
		explicit DepthTracker(XMLDocument1 * document) {
...
template<class NodeType, size_t PoolElementSize>
inline NodeType* XMLDocument1::CreateUnlinkedNode( MemPoolT<PoolElementSize>& pool )
{
...
/**
	...
	options than the XMLDocument1::Print() method.

	It can:
	-# Print to memory.
	-# Print to a file you provide.
	-# Print XML without a XMLDocument1.
...
public:
    ...
    */
    ...
    virtual bool VisitEnter( const XMLDocument1& /*doc*/ ) override;
    virtual bool VisitExit( const XMLDocument1& /*doc*/ ) override	{
        return true;
    }
